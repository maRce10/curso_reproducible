...
replicar_reg_scal <- function(itr = 100, nvars = 50, n = 30, alfa = 0.05){
  ...
  valores_p <- replicate(n = itr, expr = {
    ...
    datos <- replicate(n = nvars, expr = rnorm(n))
    ...
    mod_nulo <- lm(as.formula(frmla), data = datos_pca)
    ...
    # extraer y devolver valores de p exluyendo el del intercepto
    sm$coefficients[-1 , 4]
    
  })
  
  # calcular cuantas variables produjeron un valor de p menor a aplha para cada iteracion 
...
falsos_positivos <- replicar_reg_scal(itr = 100, nvars = nvars, alfa = 0.05)
